...
| `-r` | 重复间隔（秒） | `-r 2` |
| `-w` | 抑制警告 | `-w` |
| `-g` | 按进程组 ID 筛选 | `-g 1234` 或 `-g ^1234` |
| `-s` | 文件大小筛选 | `-s +10M` 或 `-s -1K` |
| `-b` | 避免内核阻塞（兼容，无操作） | `-b` |
| `-x` | 跨文件系统（兼容，无操作） | `-x` |
| `-S` | 避免 stat 调用 | `-S` |
| `-L` | 跟踪符号链接 | `-L` |
| `-T` | TCP/TPI 信息（队列大小） | `-T` 或 `-Tq` |
...
| 打开文件枚举（Linux） | ✅ | ✅ | 通过 `/proc/[pid]/fd` 使用 procfs |
...
- **54 个单元测试** — 过滤器解析、匹配逻辑、边界情况、TUI 导出
- **13 个 CLI 集成测试** — 输出格式、参数、表头
...
- [x] Linux 打开文件描述符枚举（`/proc/[pid]/fd`）
- [x] Linux 网络连接解析（`/proc/net/tcp|udp`）
- [x] `-L` 跟踪符号链接
- [x] `-g` 进程组筛选
- [x] `-s` 文件大小筛选
- [x] `-T` TCP/TPI 详细信息
